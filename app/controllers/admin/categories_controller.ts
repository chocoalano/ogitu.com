...
import BugReportService, { type Severity } from '#services/bug_report_service'

export default class CategoriesController {
  private moduleName = 'CategoriesController'
  ...
  /**
   * Products categories page - manages categories
   */
  async productsCategories({ inertia, request, auth, session }: HttpContext) {
    const search = request.input('search', '')
    ...
    try {
      ...
      const transformCategory = (cat: Category): any => ({
        id: cat.id,
        name: cat.name,
        slug: cat.slug,
      ...
      const totalCategories =
        categories.length + categories.reduce((sum, cat) => sum + (cat.children?.length || 0), 0)
      ...
      const activeCategories =
        ...
        categories.reduce(
          (sum, cat) => sum + (cat.children?.filter((c) => c.isActive).length || 0),
          0
        )
  ...
  async storeCategory({ request, response, session, auth }: HttpContext) {
    ...
    try {
      ...
      if (imageFile && imageFile.isValid) {
        ...
        if (uploaded) imagePath = uploaded.key
      }
   ...
   */
  async updateCategory({ params, request, response, session, auth }: HttpContext) {
    let categoryId: number | string | undefined = params.id
      ...
      if (imageFile && imageFile.isValid) {
        ...
        if (category.image) {
          try {
            ...
            await this.report(
              { request, auth },
              'updateCategory.deleteOldImage',
              deleteErr,
              { categoryId: category.id, oldImage: category.image },
              'low'
            )
          }
        }
   ...
   * Delete a category
   */
  async destroyCategory({ params, request, response, session, auth }: HttpContext) {
    ...
    try {
      ...
      if (category.image) {
        try {
          ...
          await this.report(
            { request, auth },
            'destroyCategory.deleteImage',
            deleteErr,
            { categoryId: category.id, image: category.image },
            'low'
          )
        }
      }
   ...
   */
  async toggleCategoryActive({ params, request, response, session, auth }: HttpContext) {
    const categoryId = params.id
    ...
    try {
      ...
      session.flash(
        'success',
        `Kategori berhasil di${category.isActive ? 'aktifkan' : 'nonaktifkan'}`
      )
      return response.redirect().back()
    } catch (error) {
  ...
  /**
   * Compatibility methods (kalau route kamu masih pakai yang lama)
   * -> delegasi ke method baru yang sudah ada error handling
   */
  async store(ctx: HttpContext) {
    // kalau yang lama tidak upload file, tetap aman dipanggil
  ...
  async toggleActive(ctx: HttpContext) {
