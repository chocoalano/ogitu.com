import env from '#start/env'
import axios from 'axios'
import logger from '@adonisjs/core/services/logger'

/**
 * RajaOngkir Service untuk integrasi shipping API
 * API v2 Komerce wrapper: https://rajaongkir.komerce.id/api/v1
 */
export default class RajaOngkirService {
  private apiKey: string
  private deliveryApiKey: string
  private baseUrl: string

  constructor() {
    this.apiKey = env.get('RAJAONGKIR_API_KEY_SHIPPING')
    this.deliveryApiKey = env.get('RAJAONGKIR_API_KEY_DELIVERY')
    this.baseUrl = env.get('RAJAONGKIR_BASE_URL') // https://rajaongkir.komerce.id/api/v1
  }

  /**
   * Get list of provinces
   * API v2: GET /destination/province
   */
  async getProvinces() {
    try {
      const response = await axios.get(`${this.baseUrl}/destination/province`, {
        headers: {
          key: this.apiKey,
        },
      })

      // API v2 returns data directly in 'data' key
      return response.data?.data || []
    } catch (error) {
      console.error('RajaOngkir Get Provinces Error:', error)
      return []
    }
  }

  /**
   * Get list of cities by province
   * API v2: GET /destination/city/{province_id}
   */
  async getCities(provinceId?: string) {
    try {
      if (!provinceId) {
        return []
      }

      const response = await axios.get(`${this.baseUrl}/destination/city/${provinceId}`, {
        headers: {
          key: this.apiKey,
        },
      })

      // API v2 returns data directly in 'data' key
      return response.data?.data || []
    } catch (error) {
      console.error('RajaOngkir Get Cities Error:', error)
      return []
    }
  }

  /**
   * Get list of districts/subdistricts by city
   * API v2: GET /destination/district/{city_id}
   */
  async getSubdistricts(cityId: string) {
    try {
      if (!cityId) {
        return []
      }

      const response = await axios.get(`${this.baseUrl}/destination/district/${cityId}`, {
        headers: {
          key: this.apiKey,
        },
      })

      return response.data?.data || []
    } catch (error) {
      console.error('RajaOngkir Get Districts Error:', error)
      return []
    }
  }

  /**
   * Calculate shipping cost
   * Tries multiple API endpoints for compatibility
   *
   * @param originDistrictId - District ID of origin (store location)
   * @param destinationDistrictId - District ID of destination (customer address)
   * @param weight - Weight in grams
   * @param courier - Courier code: jne, pos, tiki, jnt, sicepat, etc
   */
  async calculateCost(
    originDistrictId: string,
    destinationDistrictId: string,
    weight: number,
    courier: string
  ): Promise<any[]> {
    // Build form data
    const formData = new URLSearchParams()
    formData.append('origin', originDistrictId)
    formData.append('destination', destinationDistrictId)
    formData.append('weight', weight.toString())
    formData.append('courier', courier)

    // Try multiple endpoint patterns
    const endpoints = [
      '/calculate/district/domestic-cost', // Komerce API v1
      '/cost', // Standard RajaOngkir
      '/v1/calculate/domestic-cost', // Alternative Komerce
      '/calculate/domestic-cost', // Another variant
    ]

    for (const endpoint of endpoints) {
      try {
        logger.info(`Trying endpoint: ${this.baseUrl}${endpoint}`)

        const response = await axios.post(`${this.baseUrl}${endpoint}`, formData.toString(), {
          headers: {
            'key': this.apiKey,
            'Content-Type': 'application/x-www-form-urlencoded',
          },
        })

        logger.info(`Success with endpoint: ${endpoint}`, response.data)

        // Handle different response structures
        let services = []

        // Komerce API v2 format
        if (response.data?.data && Array.isArray(response.data.data)) {
          services = response.data.data
        }
        // Standard RajaOngkir format
        else if (response.data?.rajaongkir?.results) {
          return response.data.rajaongkir.results
        }
        // Direct array
        else if (Array.isArray(response.data)) {
          services = response.data
        }

        if (services.length === 0) {
          continue
        }

        // Filter services for the requested courier
        const courierServices = services.filter(
          (service: any) => service.code?.toLowerCase() === courier.toLowerCase()
        )

        if (courierServices.length === 0) {
          // Maybe the response is already in the standard format
          if (services[0]?.costs) {
            return services
          }
          continue
        }

        // Format to match the expected structure (same as old RajaOngkir API)
        const courierName = courierServices[0]?.name || courier.toUpperCase()

        return [
          {
            code: courier,
            name: courierName,
            costs: courierServices.map((service: any) => ({
              service: service.service || '',
              description: service.description || '',
              cost: [
                {
                  value: service.cost || 0,
                  etd: service.etd || '',
                  note: '',
                },
              ],
            })),
          },
        ]
      } catch (error) {
        if (axios.isAxiosError(error)) {
          logger.error(
            `Endpoint ${endpoint} failed:`,
            error.response?.status,
            error.response?.data?.message || error.message
          )
        }
        // Continue to next endpoint
        continue
      }
    }

    // All endpoints failed
    console.warn('All shipping cost endpoints failed', {
      origin: originDistrictId,
      destination: destinationDistrictId,
      courier,
    })

    return [
      {
        code: courier,
        name: courier.toUpperCase(),
        costs: [],
      },
    ]
  }

  /**
   * Get available shipping options for an order
   * Returns formatted options with cost and ETD
   */
  async getShippingOptions(origin: string, destination: string, weight: number) {
    const couriers = this.getCommonCouriers()
    const allOptions: any[] = []

    for (const [courierCode] of Object.entries(couriers)) {
      try {
        const results = await this.calculateCost(origin, destination, weight, courierCode)

        if (results && results.length > 0) {
          const courierData = results[0]
          if (courierData.costs && courierData.costs.length > 0) {
            const services = courierData.costs.map((cost: any) => ({
              courier_code: courierData.code,
              courier_name: courierData.name,
              service: cost.service,
              description: cost.description,
              cost: cost.cost[0]?.value || 0,
              etd: cost.cost[0]?.etd || '',
            }))
            allOptions.push(...services)
          }
        }
      } catch (error) {
        console.error(`Error fetching ${courierCode} rates:`, error)
        // Continue with other couriers
      }
    }

    return allOptions
  }

  /**
   * Get common couriers
   * API v2 doesn't provide a courier list endpoint
   */
  getCommonCouriers(): Record<string, string> {
    return {
      jne: 'JNE',
      pos: 'POS Indonesia',
      tiki: 'TIKI',
      jnt: 'J&T Express',
      sicepat: 'SiCepat',
      anteraja: 'AnterAja',
      ninja: 'Ninja Xpress',
      idexpress: 'ID Express',
      rpx: 'RPX',
      sap: 'SAP Express',
    }
  }

  /**
   * Track waybill/resi using delivery API key
   * @param waybill - Nomor resi pengiriman
   * @param courier - Courier code: jne, pos, tiki, sicepat, etc
   */
  async trackWaybill(waybill: string, courier: string) {
    try {
      const formData = new URLSearchParams()
      formData.append('waybill', waybill)
      formData.append('courier', courier)

      const response = await axios.post(`${this.baseUrl}/track/waybill`, formData.toString(), {
        headers: {
          'key': this.deliveryApiKey,
          'Content-Type': 'application/x-www-form-urlencoded',
        },
      })

      return response.data?.data || null
    } catch (error) {
      console.error('RajaOngkir Tracking Error:', error)
      return null
    }
  }

  /**
   * Get formatted tracking info
   * Returns structured tracking data with delivery status
   */
  async getTrackingInfo(waybill: string, courier: string) {
    const result = await this.trackWaybill(waybill, courier)

    if (!result) {
      return null
    }

    return {
      waybill: result.waybill,
      courier: {
        code: courier,
        name: result.summary?.courier_name || courier.toUpperCase(),
      },
      status: result.delivery_status?.status || 'UNKNOWN',
      statusDescription: result.delivery_status?.pod_receiver || '',
      shipper: result.summary?.shipper_name || '',
      receiver: result.summary?.receiver_name || '',
      origin: result.summary?.origin || '',
      destination: result.summary?.destination || '',
      history:
        result.manifest?.map((item: any) => ({
          date: item.manifest_date,
          time: item.manifest_time,
          description: item.manifest_description,
          city: item.city_name,
        })) || [],
    }
  }
}
