        ...
        for (const orderData of ordersData) {
            ...
            if (orderData.status !== 'pending_payment') {
                await Payment.create({
                    orderId: order.id,
                    paymentType: ['bank_transfer', 'gopay', 'credit_card', 'qris'][Math.floor(Math.random() * 4)],
            ...
            if (['processing', 'shipped', 'delivered'].includes(orderData.status)) {
                const totalWeight = orderItems.reduce((sum, item) => sum + item.product.weight * item.quantity, 0);
                await Shipping.create({
                    ...
                    waybill: orderData.status !== 'processing'
...
//# sourceMappingURL=order_seeder.js.map