...
export default class ProductsController {
    ...
    async index({ inertia, request, auth, session }) {
        ...
        try {
            ...
            const statsQuery = await db
                .from('products')
                .select(db.raw('COUNT(*) as total'), db.raw("SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as active"), db.raw("SUM(CASE WHEN status = 'inactive' THEN 1 ELSE 0 END) as inactive"), db.raw("SUM(CASE WHEN status = 'draft' THEN 1 ELSE 0 END) as draft"), db.raw('SUM(CASE WHEN stock = 0 THEN 1 ELSE 0 END) as out_of_stock'), db.raw('SUM(CASE WHEN stock > 0 AND stock <= 10 THEN 1 ELSE 0 END) as low_stock'))
                .first();
            const stats = {
            ...
            await this.reportAdmin({ request, auth }, 'index', error, { page, perPage, search, status, categoryId, condition, sortBy, sortDirection }, 'medium');
    ...
    async show({ params, inertia, request, auth, session }) {
    ...
    async toggleStatus({ params, request, response, session, auth }) {
        const productId = params.id;
        try {
            ...
            await product.save();
            const message = `Produk "${product.name}" berhasil di${product.status === 'active' ? 'aktifkan' : 'nonaktifkan'}`;
            if (request.header('X-Requested-With') === 'XMLHttpRequest') {
            ...
            const errorMessage = 'Gagal mengubah status produk. Silakan coba lagi atau hubungi administrator.';
            if (request.header('X-Requested-With') === 'XMLHttpRequest') {
    ...
    async toggleFeatured({ params, request, auth, response, session }) {
        const productId = params.id;
        try {
            const product = await Product.findOrFail(productId);
            product.isFeatured = !product.isFeatured;
            await product.save();
            session.flash('success', `Produk berhasil ${product.isFeatured ? 'ditambahkan ke' : 'dihapus dari'} featured`);
            return response.redirect().back();
        }
        catch (error) {
            ...
            if (product.images && product.images.length > 0) {
                for (const image of product.images) {
                    ...
                    try {
                        ...
                        await this.reportAdmin({ request, auth }, 'destroy.deleteImage', deleteErr, { productId: product.id, imageId: image.id, imageKey: image.url }, 'low');
                    }
                }
            }
    ...
    async duplicate({ params, request, response, auth }) {
        const productId = params.id;
        try {
            const product = await Product.query()
                .where('id', productId)
    ...
    async create({ inertia, request, auth, session }) {
        try {
            const categories = await Category.query().where('isActive', true).orderBy('name', 'asc');
    ...
    async store({ request, auth, response }) {
        ...
        try {
            ...
            await this.reportAdmin({ request, auth }, 'store', error, {
                name,
                sku,
    ...
    async edit({ params, inertia, request, auth, session }) {
        const productId = params.id;
        try {
            const product = await Product.query()
                .where('id', productId)
    ...
    async update({ params, request, auth, response }) {
        ...
        try {
                    ...
                    if (Array.isArray(deletedIds) && deletedIds.length > 0) {
                        ...
                        for (const image of imagesToDelete) {
                            ...
                            try {
                                ...
                                await this.reportAdmin({ request, auth }, 'update.deleteImage', deleteErr, { productId: product.id, imageId: image.id, imageKey: image.url }, 'low');
                            }
                        }
            ...
            await this.reportAdmin({ request, auth }, 'update', error, { productId, name: request.input('name'), sku: request.input('sku') }, 'high');
    ...
    async inventory({ inertia, request, auth, session }) {
        const page = request.input('page', 1);
        ...
        try {
            ...
            const statsQuery = await db
                .from('products')
                .select(db.raw('COUNT(*) as total_products'), db.raw('SUM(CASE WHEN stock > 10 THEN 1 ELSE 0 END) as in_stock'), db.raw('SUM(CASE WHEN stock > 0 AND stock <= 10 THEN 1 ELSE 0 END) as low_stock'), db.raw('SUM(CASE WHEN stock <= 0 THEN 1 ELSE 0 END) as out_of_stock'))
                .first();
            ...
            await this.reportAdmin({ request, auth }, 'inventory', error, { page, perPage, search, status, categoryId, stockStatus }, 'medium');
    ...
    async updateStock({ request, auth, response }) {
        try {
            ...
            await this.reportAdmin({ request, auth }, 'updateStock', error, {
