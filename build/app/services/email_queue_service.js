...
class EmailQueueService {
    ...
    async queueOtpEmail(email, code, type) {
        const { subject, html } = this.getOtpEmailTemplate(code, type);
        const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_OTP, {
            to: email,
            subject,
            html,
            priority: 'high',
    ...
    async queueWelcomeEmail(email, name) {
        const { subject, html } = this.getWelcomeEmailTemplate(name);
        const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_WELCOME, {
            to: email,
            subject,
            html,
            priority: 'normal',
    ...
    async queueOrderConfirmationEmail(email, orderId, orderDetails) {
        const { subject, html } = this.getOrderConfirmationTemplate(orderId, orderDetails);
        const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_ORDER_CONFIRMATION, {
            to: email,
            subject,
            html,
            priority: 'high',
    ...
    async queueOrderStatusEmail(email, orderId, status, trackingNumber) {
        const { subject, html } = this.getOrderStatusTemplate(orderId, status, trackingNumber);
        const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_ORDER_STATUS_UPDATE, {
            to: email,
            subject,
            html,
            priority: 'normal',
    ...
    async queueEmail(data) {
        const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_GENERIC, data);
        return job.id;
    }
    async queueBulkEmails(recipients, subject, html, options) {
        const jobIds = [];
        let index = 0;
        for (const recipient of recipients) {
            const job = await queueService.addJob(EMAIL_QUEUE, EMAIL_JOBS.SEND_NEWSLETTER, {
                to: recipient,
                subject,
                html,
                priority: 'low',
            }, {
                delay: options?.delay ? options.delay * index : index * 100,
            });
            jobIds.push(job.id);
            index++;
        }
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f3f4f6;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    ...
    getOrderConfirmationTemplate(orderId, orderDetails) {
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f3f4f6;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    ...
    getOrderStatusTemplate(orderId, status, trackingNumber) {
        const statusColors = {
            processing: '#f59e0b',
            shipped: '#3b82f6',
            delivered: '#10b981',
...
//# sourceMappingURL=email_queue_service.js.map