...
// Register Transmit routes for SSE (Server-Sent Events)
...
// Public routes (with public rate limiting)
...
// Auth routes (guest only) with auth rate limiting
router
...
// Customer protected pages (with throttle)
router
...
// Customer protected API routes (with rate limiting)
router
  .group(() => {
    // Cart routes (with customer API throttle)
    ...
    // Location routes (frequently accessed)
...
// Checkout & Payment routes (with stricter rate limiting)
router
  .group(() => {
