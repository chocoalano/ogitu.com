...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.decimal('value', 10, 2).notNullable() // percentage, fixed amount, or 0 for free shipping
      ...
      table.boolean('is_public').defaultTo(true) // if false, only shared via link
      table.boolean('applies_to_all').defaultTo(true) // applies to all products
      table.timestamp('created_at')
      table.timestamp('updated_at')
    })

    // Create voucher_products pivot table for specific products
    ...
    // Create voucher_usages table to track customer usage
