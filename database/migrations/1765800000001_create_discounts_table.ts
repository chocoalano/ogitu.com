...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.decimal('value', 10, 2).notNullable() // percentage or fixed amount
      table.decimal('min_purchase', 12, 2).defaultTo(0) // minimum purchase amount
      ...
      table.boolean('applies_to_all').defaultTo(false) // applies to all products
      table.timestamp('created_at')
      table.timestamp('updated_at')
    })

    // Create discount_products pivot table for specific products
